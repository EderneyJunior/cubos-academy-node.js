...
--7 - Faça uma query que retorna a soma do estoque disponível dos medicamentos 
...
--9 - Faça uma query que retorna uma única coluna com a junção do nome do medicamentoe a categoria 
--entre parenteses de todos os registros em que a categoria não seja nula da tabela 
--`farmacia`. Ex.: `Endocet (green)`.
...
--na tabela `usuarios`. O tempo precisa ser uma coluna que retorne a quantidade de anos, meses, dias,
...
--possui cadastrado na tabela `usuarios`. O tempo precisa ser uma coluna que retorne apenas a quantidade de 
--anos, meses, dias em um objeto. Ex.: `{"years":2,"months":2,"days":19}`.

select nome, idade, email, age(cast(cadastro as date)) from usuarios where idade >= 60;


--14 - Faça uma query que retorna a categoria e a quantidade de produtos de cada categoria 
